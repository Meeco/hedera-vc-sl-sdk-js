...
export type PublicKeyProvider<T extends HcsVcMessage> = (evn: MessageEnvelope<T>) => PublicKey;
 ...
 */
export class MessageEnvelope<T extends HcsVcMessage> {
    ...
    constructor(...args: any[]) {
        if (args.length === 0) {
            ...
            throw new VCError("Wrong arguments passed to constructor");
        }
    }
    ...
    public sign(signer: SignFunction): Uint8Array {
        if (!signer) {
            throw new VCError("Signing function is not provided.");
        }

        if (this.signature) {
            throw new VCError("Message is already signed.");
        }
     ...
     */
    public static fromMirrorResponse<U extends HcsVcMessage>(
     ...
     * @return The {@link MessageEnvelope}.
     */
    public static fromJson<U extends HcsVcMessage>(json: string, messageClass: JsonClass<U>): MessageEnvelope<U> {
