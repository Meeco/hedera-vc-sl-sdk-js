...
export class W3CCredential<T extends AnyCredentialSubject> implements Verifiable<CredentialPayload> {
    ...
    public static fromJWT(jwt: string): W3CCredential<any> {
        ...
        if (!isCanonical(header) || !isCanonical(payload)) {
            throw new Error("decoded JWT not in canonical form, conversion to W3C format will break the signature");
        }
        ...
        if (!payload.vc || !payload.vc.credentialSubject) {
            throw new Error('VC JWT did not contain a "credentialSubject" -- is it a VP?');
        }
        ...
        const result = {
            ...
            proof: {
                ...
                verificationMethod: header?.kid || vcPartial.issuer.id + "#did-root-key",
                jws: signature,
            },
        };
    ...
    public static async createJWT(
        ...
        return createJWS(parsedPayload, signer.signer, { alg: "EdDSA", typ: "JWT" }, { canonicalize: true });
    }
     ...
     */
    constructor(credential: Verifiable<W3CCredentialExternal | CredentialPayload>, originalJWT?: string) {
